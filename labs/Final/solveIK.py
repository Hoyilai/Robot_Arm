...
class IK:
    ...
    def __init__(self,linear_tol=1e-4, angular_tol=1e-3, max_steps=75, min_step_size=1e-5):
        """
    ...
    def displacement_and_axis(target, current):
        ...
        #from current to ee frame
        #R*R_c = R_e
        R = target@np.linalg.inv(current)
    ...
    def is_valid_solution(self,q,target):
        ...
        dis, angle = IK.distance_and_angle(pose,target)
        angle_d = angle*180/np.pi
        print("distance = %f, angle = %f" % (dis, angle_d))

        if (dis > self.linear_tol):
            return False, dis, angle_d
        elif (angle > self.angular_tol):
            return False, dis, angle_d

        ## END STUDENT CODE

        return True, dis, angle_d
    ...
    def end_effector_task(q,target):
        ...
        joints, pose = IK.fk.forward(q)#get current/target end effector pose
        disp, axis = IK.displacement_and_axis(target,pose)
        vel = np.concatenate((disp,axis))

        J = calcJacobian(q)
        dq = np.linalg.pinv(J) @ vel
    ...
    def inverse(self, target, seed):
        ...
        while True:
            ...
            q_sec = np.zeros(7)
            q_sec = (np.eye(7) - np.linalg.pinv(J)@J) @ dq_center
            ...
            new_q = q + dq
            for i in range(len(IK.lower)):
                if (new_q[i] < IK.lower[i]):
                    new_q[i] = IK.lower[i]

            for j in range(len(IK.upper)):
                if (new_q[j] > IK.upper[j]):
                    new_q[j] = IK.upper[j]

            q = new_q

        success, dis, angle = self.is_valid_solution(q,target)
        return q, dis, angle, success, rollout
